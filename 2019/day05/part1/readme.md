...
The Thermal Environment Supervision Terminal (TEST) starts by running a __diagnostic program__ (your puzzle input). The TEST diagnostic program will run on [your existing Intcode computer](../../intcode.py) after a few modifications:
...
Each parameter of an instruction is handled based on its parameter mode. Right now, your ship computer already understands parameter mode `0`, __position mode__, which causes the parameter to be interpreted as a __position__ - if the parameter is `50`, its value is __the value stored at address `50`__ in __memory__. Until now, all parameters have been in position mode.
...
After providing `1` to the only input instruction and passing all the tests, __what diagnostic code does the program produce__?
