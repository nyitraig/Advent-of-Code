...
var lengths = Array.from(input).map(x => x.charCodeAt(0)).concat([17, 31, 73, 47, 23]);
var pos = 0, skip = 0;
...
for (var i = 0; i < 64; i++) {
	lengths.forEach(length => {
		for (var i = 0; i < length / 2; i++) {
			var x = (pos + i) % list.length, y = (pos + length - 1 - i) % list.length;
...
console.log(dense.map(e => e < 16 ? '0' + e.toString(16) : e.toString(16)).join(''));
